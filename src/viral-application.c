...
#include "viral-application.h"
#include "viral-window.h"

struct _ViralApplication
{
    AdwApplication parent_instance;
};

G_DEFINE_TYPE(ViralApplication, viral_application, ADW_TYPE_APPLICATION)
...
static void viral_application_try_exit (GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
...
static void viral_application_close (GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
    GtkWindow *window = gtk_application_get_active_window(GTK_APPLICATION(user_data));
...
/*static void viral_application_open_drive(GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
    GtkWindow *window = gtk_application_get_active_window(GTK_APPLICATION(user_data));
    ...
    viral_window_open(g_file_new_for_path(path), window, name);
...
static void viral_application_show_about(GSimpleAction *action, GVariant *parameter, gpointer user_data)
{
    ...
    {
        "Joshua Murphy https://github.com/ranguli",
    ...
    g_autofree gchar *program_name = g_strdup(_("Viral"));

    adw_show_about_window (window,
                         "transient-for", window,
                         "application-name", program_name,
                         "application-icon", "com.github.ranguli.Viral",
                         "version", "2.0.0",
                         "copyright", "© 2022 Joshua Murphy",
                         "issue-url", "https://github.com/ranguli/viral/issues/new",
                         "license-type", GTK_LICENSE_GPL_3_0,
                         "developer-name", "Joshua Murphy",
...
static void viral_application_open(GApplication *application, GFile **files, gint n_files, const gchar *hint)
{
    ...
    viral_window_open_files(window, file_list);
    gtk_window_present(GTK_WINDOW(window));
}

static void viral_application_finalize(GObject *object)
{
    G_OBJECT_CLASS(viral_application_parent_class)->finalize(object);
}

static void viral_application_activate(GApplication *app)
{
...
static void viral_application_class_init(ViralApplicationClass *klass)
{
    GObjectClass *object_class = G_OBJECT_CLASS(klass);
    GApplicationClass *app_class = G_APPLICATION_CLASS(klass);

    object_class->finalize = viral_application_finalize;
    app_class->activate = viral_application_activate;
    app_class->open = viral_application_open;
}

static void viral_application_init(ViralApplication *self)
{
    g_autoptr(GSimpleAction) quit_action = g_simple_action_new("quit", NULL);
    g_signal_connect(quit_action, "activate", G_CALLBACK(viral_application_try_exit), self);
    ...
    g_signal_connect(close_action, "activate", G_CALLBACK(viral_application_close), self);
    ...
    g_signal_connect(about_action, "activate", G_CALLBACK(viral_application_show_about), self);
    g_action_map_add_action(G_ACTION_MAP(self), G_ACTION(about_action));

    g_autoptr(GSimpleAction) new_window_action = g_simple_action_new("new-window", NULL);
    g_signal_connect(new_window_action, "activate", G_CALLBACK(viral_new_window), self);
    ...
    g_signal_connect(open_action, "activate", G_CALLBACK(viral_application_open_to_window), self);
    ...
    g_signal_connect(open_drive_action, "activate", G_CALLBACK(viral_application_open_drive), self);
