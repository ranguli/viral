...
static void viral_window_init(ViralWindow *self)
{
    ...
    g_signal_connect(self->shred_button, "clicked", G_CALLBACK(viral_window_start_shredding), self);
    g_signal_connect(self->abort_button, "clicked", G_CALLBACK(viral_window_abort_shredding), self);
    g_signal_connect(self, "close-request", G_CALLBACK(viral_window_exit), NULL);
...
static gboolean on_drop(GtkDropTarget *target, const GValue *value, double x, double y, gpointer data)
{
    ...
    viral_window_open_files(data, file_list);

    return TRUE;
}

static void viral_window_exit_response(GtkDialog *dialog, gchar *response, ViralWindow *self)
{
    if (g_strcmp0(response, "exit") == 0)
    {
        ...
        viral_window_abort_shredding(NULL, GTK_WIDGET(self));
    }
}

gboolean viral_window_exit(ViralWindow *win, gpointer data)
{
    if (win->status)
    {
...
static void viral_window_open_files_finish(GObject *source_object, GAsyncResult *res, gpointer user_data)
{
}
static void viral_window_open_files_thread(GTask *task, gpointer source_object, gpointer task_data, GCancellable *cancellable)
{
    ...
    {
        gboolean cont = viral_window_open_file(l->data, window, NULL); // This adds an entry to the current window.
        if (cont == FALSE)
            break;
    }
    // viral_window_open_file_finish() is called here.
}
void viral_window_open_files(ViralWindow *window, GList *file_list)
{
    GTask *task = g_task_new(window, NULL, viral_window_open_files_finish, window);
    g_task_set_task_data(task, file_list, NULL);
    g_task_run_in_thread(task, viral_window_open_files_thread);
    g_object_unref(task);
}
...
gboolean viral_window_open_file(GFile *file, gpointer data, gchar *title)
{
    ...
    GtkWidget *file_row = GTK_WIDGET(viral_file_row_new(file));
...
static void viral_window_shred_files_thread(GTask *task, gpointer source_object, gpointer task_data, GCancellable *cancellable)
{
    ...
    // viral_window_shred_file_finish() is called here.
}
static void viral_window_start_shredding(GtkWidget *widget, gpointer data)
{
    ViralWindow *window = RAIDER_WINDOW(data);
    ...
    GTask *task = g_task_new(window, NULL, viral_window_shred_files_finish, window);
    g_task_run_in_thread(task, viral_window_shred_files_thread);
    g_object_unref(task);
}
...
static void viral_window_abort_shredding(GtkWidget *widget, gpointer data)
{
    ...
    GTask *task = g_task_new(window, NULL, viral_window_abort_files_finish, datai);
    g_task_run_in_thread(task, viral_window_abort_files_thread);
    g_object_unref(task);
}
...
/*void on_mount_changed(gpointer object, gpointer monitor, gpointer data)
{
    ...
    for (l = mount_list; l != NULL; l = l->next)
    {
        ...
        GVariant *var = g_variant_new_string(g_file_get_path(file)); // Store in a variant so the "action" in viral-application can know which drive we are working on.
